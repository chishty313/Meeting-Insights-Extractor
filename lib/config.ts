...
export const config = {
  ...
  azure: {
    apiKey: process.env.AZURE_OPENAI_API_KEY || 
            (typeof window !== 'undefined' && (window as any).__ENV_AZURE_OPENAI_API_KEY__) ||
            (typeof window !== 'undefined' && (window as any).AZURE_OPENAI_API_KEY) ||
            '',
    endpoint: process.env.AZURE_OPENAI_ENDPOINT || 
              (typeof window !== 'undefined' && (window as any).__ENV_AZURE_OPENAI_ENDPOINT__) ||
              (typeof window !== 'undefined' && (window as any).AZURE_OPENAI_ENDPOINT) ||
              '',
  },
  ...
  zoom: {
    clientId: process.env.ZOOM_CLIENT_ID || 
              (typeof window !== 'undefined' && (window as any).__ENV_ZOOM_CLIENT_ID__) ||
              (typeof window !== 'undefined' && (window as any).ZOOM_CLIENT_ID) ||
              '',
    clientSecret: process.env.ZOOM_CLIENT_SECRET || 
                  (typeof window !== 'undefined' && (window as any).__ENV_ZOOM_CLIENT_SECRET__) ||
                  (typeof window !== 'undefined' && (window as any).ZOOM_CLIENT_SECRET) ||
                  '',
    accountId: process.env.ZOOM_ACCOUNT_ID || 
               (typeof window !== 'undefined' && (window as any).__ENV_ZOOM_ACCOUNT_ID__) ||
...
export const isConfigured = {
  azure: (() => {
    ...
    console.log('  - API Key:', hasKey ? `SET (${config.azure.apiKey.substring(0, 10)}...)` : 'NOT SET');
    ...
    console.log('  - Process.env.AZURE_OPENAI_ENDPOINT:', !!process.env.AZURE_OPENAI_ENDPOINT);
    
    if (typeof window !== 'undefined') {
      console.log('  - Window.__ENV_AZURE_OPENAI_API_KEY__:', !!(window as any).__ENV_AZURE_OPENAI_API_KEY__);
      console.log('  - Window.__ENV_AZURE_OPENAI_ENDPOINT__:', !!(window as any).__ENV_AZURE_OPENAI_ENDPOINT__);
    }
  ...
  zoom: !!(config.zoom.clientId && config.zoom.clientSecret && config.zoom.accountId),
};
